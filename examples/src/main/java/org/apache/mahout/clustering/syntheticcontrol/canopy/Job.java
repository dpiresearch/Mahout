...
public final class Job extends AbstractJob {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length > 0) {
      ...
      String queueName = "hdmi-others";
      String propQueueName = "mapred.job.queue.name";
//      log.info("Setting queue name "+propQueueName+ " to "+queueName);
      Job job = new Job();
      org.apache.hadoop.conf.Configuration config = job.getConf();
      if (config == null) {
          config = new Configuration();
      }
      config.set(propQueueName, queueName);
//      ToolRunner.run(new Configuration(), new Job(), args);
      ToolRunner.run(config, new Job(), args);
//      ToolRunner.run(null, new Job(), args);
    } else {
...
//      HadoopUtil.delete(new Configuration(), output);
      HadoopUtil.delete(config, output);
  ...
  private static void run(Path input, Path output, DistanceMeasure measure,
    ...
    String queueName = "hdmi-others";
    String propQueueName = "mapred.job.queue.name";
    log.info("In run method 101: Setting queue name "+propQueueName+ " to "+queueName);
    org.apache.hadoop.conf.Configuration config = new Configuration();
    config.set(propQueueName,queueName);
    CanopyDriver.run(config, directoryContainingConvertedInput,
